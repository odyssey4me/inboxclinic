...
| 2026-08-15 | `healthInputFromSenders` also resolves the rules **covering** a sender from above (a parent-domain rule, or an ancestor domain-scope block), matching every other effective-status call site — a sender decided one level up now counts as decided rather than pending (#251). | Claude |
