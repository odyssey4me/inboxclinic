...
| 2026-07-16 | Prior-decisions import becomes **per-suggestion, not all-or-nothing**: each row picks address/domain/skip independently before "Import selected"; a suggestion can be escalated from address to its whole domain. `learnPriorDecisions` now carries `unreadShare` for trash-derived suggestions so the read-weighted signal is shown (e.g. "82% unread of 3 messages") instead of collapsing to a flat "binned unread" assumption. | Claude |
