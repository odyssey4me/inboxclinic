...
are also URL-controlled, so they're bookmarkable and back/forward moves between them; every setter
merges params so `?demo=1` survives (#120).
...
| 2026-07-18 | **#120 — deep-link the decisions surface:** the home surface's active tab (`?tab=decided\|all`) and an open detail (`?sender=<id>` / `?domain=<id>`) are now **URL-controlled** via `useSearchParams`, so they're bookmarkable and back/forward moves between them; a deep link opens that tab/detail on load and closing clears the param. Every setter merges params so `?demo=1` survives. Closes the tab/detail follow-up left by #102. | Claude |
