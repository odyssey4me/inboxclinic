...
A **domain**-scope block also steps aside for a **subdomain** the user separately trusted, which
is not recorded in `exceptionDomains` but is carved out of the compiled filter as a
`*@sub.domain` term (design-gmail-integration.md Decision 5 point 9). Resolution reads that
carve-out the same way, so the status and the filter cannot disagree about who is covered.

A **parent-domain** rule does not: Decision 9 makes it the later, broader word over a subdomain
trust it has not carved out, so it spares only the subdomains in its own `exceptionDomains`
(#250). The two scopes differ because the questions differ — nothing carves out retroactively,
so a subdomain trusted *before* a rule was made above it was never carved out of that rule.
...
6. **UX** — offer it *in context*, like the flagged-siblings offer: while deciding on a subdomain
   ...
   **After the decision, the rule must keep explaining itself.** Breadth stated once, at the moment
   of deciding, is not enough: a subject met later shows a status nothing in the UI accounts for.
   So a subject governed by a rule from above **names that rule** wherever it is shown — and a
   subject carved out of one is offered the way back (`withdrawDecision`). The weight
   differs by level: `DomainDetail` carries a panel, because a domain's own carve-out action has no
   other home; `SenderDetail` carries a single line, because making the carve-out there is what the
   address-scoped Trust/Block buttons already do, so only *rejoining* needs a control of its own.
...
type DecisionScope = 'address' | 'domain' | 'parentDomain'; // 'parentDomain' = Decision 9 (#136)
...
```ts
/**
 * Pure precedence rule: most-specific-wins — address exception → exact domain → parent domain.
 * `parentDomainStatus`/`'parentDomain'` are the Decision 9 (#136) additions; a `parentDomain`
 * decision applies when the sender's registrable domain (eTLD+1, via `tldts`) equals the rule's.
 */
function resolveEffectiveDecision(input: {
  ...
  parentDomainStatus: TrustStatus | null; // #136 — the eTLD+1 rule's status, if any
}): { status: TrustStatus; source: 'address' | 'domain' | 'parentDomain' | 'none' };
```
